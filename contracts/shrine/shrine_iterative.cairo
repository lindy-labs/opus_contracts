...
from contracts.lib.types import Trove, Yang, YangPendingDebtInfo
...
// Mapping from a yang ID to its pending debt info (total, last redistribution's error)
@storage_var
func shrine_yang_pending_debt_info(yang_id: ufelt) -> (info: packed) {
}
...
func redistribute{
    ...
    let redistributed_debt: wad = redistribute_internal(
        redistribution_id, trove_id, trove_value, old_trove_info.debt, yang_count, interval, 0
    );

    with_attr error_message("Shrine: Redistributed debt is not equal to trove's debt") {
        assert redistributed_debt = old_trove_info.debt;
    }
...
func redistribute_internal{
    ...
    let new_error: wad = WadRay.sub_unsigned(
        adjusted_debt_to_distribute, WadRay.wmul(debt_increment_per_yang, new_yang_total)
    );
    let updated_pending_debt_info: YangPendingDebtInfo = YangPendingDebtInfo(new_total, new_error);
    set_pending_debt_info(current_yang_id, updated_pending_debt_info);
...
func round_distributed_debt{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let round_up: bool = is_le(remaining_debt, WadRay.DIFF);
    if (round_up == TRUE) {
        return (
...
func pull_pending_debt_for_trove_internal{
    ...
    if (update_state == TRUE) {
        let new_yang_pending_debt_info: YangPendingDebtInfo = YangPendingDebtInfo(
            old_yang_pending_debt_info.total - debt_increment, old_yang_pending_debt_info.last_error
        );
        set_pending_debt_info(current_yang_id, new_yang_pending_debt_info);
...
func get_yang_pending_debt_info_internal{
    ...
    let (total: wad, last_error: wad) = split_felt(pending_debt_info_packed);
    let pending_debt_info: YangPendingDebtInfo = YangPendingDebtInfo(
        total=total, last_error=last_error
    );
    return (pending_debt_info,);
}

func set_pending_debt_info{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    yang_id: ufelt, pending_debt_info: YangPendingDebtInfo
) {
    let packed_pending_debt_info: packed = pack_felt(
        pending_debt_info.total, pending_debt_info.last_error
    );
    shrine_yang_pending_debt_info.write(yang_id, packed_pending_debt_info);
    return ();
}
