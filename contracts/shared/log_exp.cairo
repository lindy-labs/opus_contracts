...
# Log and exponential functions library for wads
namespace LogExp:
    ...
    # (2^125 - 1) / 10^20, which makes the largest exponent ln((2^125 - 1) / 10^20) = 130.700829182905140221.
    # The smallest possible result is 10^(-18), which makes largest negative argument
    # ln(10^(-18)) = -41.446531673892822312.
    # We use 130.0 and -41.0 to have some safety margin.
    const MAX_NATURAL_EXPONENT = 40 * 10 ** 18
    const MIN_NATURAL_EXPONENT = -41 * 10 ** 18

    const LN_36_LOWER_BOUND = ONE_18 - 10 ** 17
    const LN_36_UPPER_BOUND = ONE_18 + 10 ** 17 

    const ARG_BOUND = 2 ** 125
    ...
    func assert_acceptable{range_check_ptr}(x):
        assert_le(x, ARG_BOUND)
        assert_le(0, x)
    end

    # Exponentiation (x^y) with unsigned 18 decimal fixed point base and exponent
    # Reverts if ln(x) * y is smaller than `MIN_NATURAL_EXPONENT`, or larger than `MAX_NATURAL_EXPONENT`.
    func pow{range_check_ptr}(x, y) -> (res):
        alloc_locals

        # This implies that 0^0 = 1
        if y == 0:
            return (ONE_18)
        end 
        ...
        # Asserting x and y are in range 
        assert_acceptable(x)
        assert_acceptable(y) 

        let (x_gt_ln36_lower_bound) = is_le(LN_36_LOWER_BOUND, x - 1) 
        let (x_lt_ln36_upper_bound) = is_le(x, LN_36_UPPER_BOUND - 1) 

        let x_in_ln36_range = x_gt_ln36_lower_bound * x_lt_ln36_upper_bound # If either of the factors are zero, then `x_in_ln36_range` will be 0 

        if x_in_ln36_range == TRUE:
            #let (ln_36_x) = ln_36_internal(x)
            let ln_36_x = 1
            let (first_half,second_half) = unsigned_div_rem(ln_36_x, ONE_18)

            let (second_half_times_y, _) = unsigned_div_rem(second_half * y, ONE_18)

            let logx_times_y = first_half * y + second_half_times_y

        else:
            #let (ln_x) = ln_internal(x)
            let ln_x = 1
            let logx_times_y = ln_x * y 
        end

        let (logx_times_y, _) = unsigned_div_rem(logx_times_y, ONE_18) # Truncating down to 18 decimals 
    ...
    func exp{range_check_ptr}(x) -> (res):
        ...
        assert_le(MIN_NATURAL_EXPONENT, x)
        ...
        let x = x * 100 # Scaling x from an 18 decimal number to a 20 decimal number to enhance precision 
