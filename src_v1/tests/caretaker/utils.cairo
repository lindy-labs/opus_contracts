mod CaretakerUtils {
    ...
    fn caretaker_deploy() -> (
        ...
        IAccessControlDispatcher { contract_address: shrine.contract_address }.grant_role(ShrineRoles::caretaker(), caretaker);

        // allow Caretaker to call exit in Sentinel during shut
        set_contract_address(SentinelUtils::admin());
        IAccessControlDispatcher { contract_address: sentinel.contract_address }.grant_role(SentinelRoles::caretaker(), caretaker);
    ...
    fn only_eth(yangs: Span<ContractAddress>, gates: Span<IGateDispatcher>) -> (Span<ContractAddress>, Span<IGateDispatcher>, Span<u128>) {
        let mut eth_yang = Default::default();
        eth_yang.append(*yangs[0]);
        let mut eth_gate = Default::default();
        eth_gate.append(*gates[0]);
        let mut eth_amount = Default::default();
        eth_amount.append(AbbotUtils::ETH_DEPOSIT_AMT);
