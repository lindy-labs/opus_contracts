...
mod Pragma {
    ...
    #[event]
    fn YangAdded(index: u64, settings: YangSettings) {}
    ...
    fn set_oracle(new_oracle: ContractAddress) {
        ...
        assert(new_oracle.is_non_zero(), 'Address cannot be zero');
    ...
    fn set_price_validity_thresholds(freshness: u64, sources: u64) {
        ...
        assert(
            ...
            'Freshness out of bounds'
        );
        assert(
            LOWER_SOURCES_BOUND <= sources & sources <= UPPER_SOURCES_BOUND, 'Sources out of bounds'
        );
    ...
    fn update_prices() {
        ...
        assert(can_force_price_update, 'too soon to update prices');
    ...
    #[external]
    fn probe_task() -> bool {
        let seconds_since_last_update: u64 = get_block_timestamp() - last_price_update::read();
    ...
    fn assert_new_yang(yang: ContractAddress) {
        let mut idx: u64 = 0;
        let yangs_count: u64 = yangs_count::read();
        ...
        loop {
            ...
            assert(settings.yang != yang, 'Yang already present');
            idx += 1;
        };
    }

    fn update_prices_internal() {
        ...
        last_price_update::write(block_timestamp);
