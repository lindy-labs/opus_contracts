mod test_purger {
    ...
    use opus::tests::purger::flash_liquidator::{IFlashLiquidatorDispatcher, IFlashLiquidatorDispatcherTrait};
    ...
    fn test_set_penalty_scalar_pass() {
        ...
        common::assert_events_emitted(purger.contract_address, expected_events.span(), Option::None);
    }
    ...
    fn test_liquidate_with_flashmint_pass() {
        ...
        let flash_liquidator = purger_utils::flash_liquidator_deploy(
            shrine.contract_address, abbot.contract_address, flashmint.contract_address, purger.contract_address
        );
    ...
    fn test_full_absorb_pass() {
        ...
        let purged_event: purger_contract::Purged = common::pop_event_with_indexed_keys(purger.contract_address)
            .unwrap();
        common::assert_asset_balances_equalish(
        ...
        let compensate_event: purger_contract::Compensate = common::pop_event_with_indexed_keys(purger.contract_address)
            .unwrap();
        assert(
                                                ...
                                                match is_recovery_mode_fuzz.pop_front() {
                                                    Option::Some(is_recovery_mode) => {
                                                        ...
                                                        loop {
                                                            ...
                                                            // Sanity check that some interest has accrued
                                                            assert(
                                                                accrued_interest.is_non_zero(), 'no interest accrued'
                                                            );
                                                            ...
                                                            abbot
                                                                .forge(
                                                                    recipient_trove, max_close_amt, WadZeroable::zero()
                                                                );
                                                            ...
                                                            absorber_start_yin -= absorber_contract::MINIMUM_SHARES
                                                                .into();
                                ...
                                loop {
                                    match is_recovery_mode_fuzz.pop_front() {
                                        Option::Some(is_recovery_mode) => {
                                            ...
                                            // Fund the absorber with twice the target trove's debt
                                            let absorber_start_yin: Wad = (target_trove_start_health.debt.val * 2)
                                                .into();
                                            ...
                                            assert(
                                                shrine
                                                    .get_yin(absorber.contract_address) > target_trove_start_health
                                                    .debt,
                                                'not full absorption'
                                            );
                                            ...
                                            if *is_recovery_mode {
                                                set_contract_address(purger_utils::target_trove_owner());
                                                abbot.close_trove(whale_trove);
                                    ...
                                    match is_recovery_mode_fuzz.pop_front() {
                                        Option::Some(is_recovery_mode) => {
                                            ...
                                            let target_trove_after_health: Health = shrine
                                                .get_trove_health(target_trove);
                                            assert(
                                                target_trove_after_health.ltv.is_zero(), 'wrong LTV after liquidation'
                                            );
                                            assert(
                                                target_trove_after_health.value.is_zero(),
                                                'wrong value after liquidation'
