mod test_purger {
    ...
    fn test_partial_absorb_with_redistribution_entire_trove_debt(
        ...
        for target_trove_yang_asset_amts in target_trove_yang_asset_amts_cases {
            ...
            for yang_asset_amts in recipient_trove_yang_asset_amts_cases {
                let initial_trove_debt: Wad = purger_utils::TARGET_TROVE_YIN.into();
                let absorber_yin_cases: Span<Wad> = purger_utils::generate_operational_absorber_yin_cases(
                    initial_trove_debt,
                );
                    ...
                    for is_recovery_mode in is_recovery_mode_fuzz {
                        for kill_absorber in kill_absorber_fuzz {
                            ...
                            if is_full_exceptional_redistribution {
                                ...
                                assert(
                                    recipient_trove_after_health.debt > recipient_trove_start_health.debt,
                            ...
                            assert(purged_event.percentage_freed == expected_freed_pct, 'wrong Purged freed pct');
                            assert(purged_event.funder == absorber.contract_address, 'wrong Purged funder');
                            assert(purged_event.recipient == absorber.contract_address, 'wrong Purged recipient');
    ...
    // provided yin yet.
    fn test_absorb_full_redistribution(recipient_trove_yang_asset_amts: Span<Span<u128>>) {
        let classes = Option::Some(purger_utils::declare_contracts());
