...
mod TestFlashmint {
    ...
    fn flash_borrower_deploy() -> ContractAddress {
        let mut calldata = ArrayTrait::new(); 
    ...
    fn flash_borrower_setup() -> (ContractAddress, IFlashMintDispatcher, ContractAddress) {
        ...
        (shrine, flashmint, flash_borrower_deploy())
    }
    ...
    fn test_flashmint_pass() {
        ...
        flashmint.flash_loan(borrower, shrine, 0_u256, calldata);
        flashmint.flash_loan(borrower, shrine, WAD_ONE.into(), calldata);
    ...
    #[test]
    #[available_gas(20000000000)]
    //#[should_panic(expected: ('', 'ENTRYPOINT_FAILED'))] TODO: get the error message
    ...
    #[test]
    #[available_gas(20000000000)]
    //#[should_panic(expected: ('RG: reentrant call', 'ENTRYPOINT_FAILED'))] TODO: get the error message
...
mod FlashBorrower {
    ...
    fn on_flash_loan(
        ...
        if action == ATTEMPT_TO_STEAL {
            ...
            IFlashMintDispatcher{contract_address: token}.flash_loan(initiator, token, amount, call_data_copy);
