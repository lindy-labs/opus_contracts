...
mod Absorber {
    ...
    impl IAbsorberImpl of IAbsorber<Storage> {
        ...
        // Getters
        //
        fn get_rewards_count(self: @Storage) -> u8 {
            self.rewards_count.read()
        }

        fn get_rewards(self: @Storage) -> Span<Reward> {
        ...
        fn get_current_epoch(self: @Storage) -> u32 {
            self.current_epoch.read()
        }

        fn get_absorptions_count(self: @Storage) -> u32 {
            self.absorptions_count.read()
        }

        fn get_absorption_epoch(self: @Storage, absorption_id: u32) -> u32 {
            self.absorption_epoch.read(absorption_id)
        }

        fn get_total_shares_for_current_epoch(self: @Storage) -> Wad {
            self.total_shares.read()
        }

        fn get_provision(self: @Storage, provider: ContractAddress) -> Provision {
            self.provisions.read(provider)
        }

        fn get_provider_last_absorption(self: @Storage, provider: ContractAddress) -> u32 {
            self.provider_last_absorption.read(provider)
        }

        fn get_provider_request(self: @Storage, provider: ContractAddress) -> Request {
            self.provider_request.read(provider)
        }

        fn get_asset_absorption(
            self: @Storage, asset: ContractAddress, absorption_id: u32
        ) -> DistributionInfo {
            self.asset_absorption.read((asset, absorption_id))
        }

        fn get_cumulative_reward_amt_by_epoch(
            self: @Storage, asset: ContractAddress, epoch: u32
        ) -> DistributionInfo {
            self.cumulative_reward_amt_by_epoch.read((asset, epoch))
        }

        fn get_provider_last_reward_cumulative(
            self: @Storage, provider: ContractAddress, asset: ContractAddress
        ) -> u128 {
            self.provider_last_reward_cumulative.read((provider, asset))
        }

        fn get_removal_limit(self: @Storage) -> Ray {
        ...
        fn preview_remove(self: @Storage, provider: ContractAddress) -> Wad {
        ...
        fn preview_reap(
            self: @Storage, provider: ContractAddress
        ) -> (Span<ContractAddress>, Span<u128>, Span<ContractAddress>, Span<u128>) {
            let provision: Provision = self.provisions.read(provider);
            let provider_last_absorption_id: u32 = self.provider_last_absorption.read(provider);
            let current_absorption_id: u32 = self.absorptions_count.read();

            let (absorbed_assets, absorbed_asset_amts) = self
            ...
            let updated_reward_amts: Span<u128> = self
        ...
        fn provide(ref self: Storage, amount: Wad) {
            ...
            let provision: Provision = self.provisions.read(provider);
            self.reap_internal(provider, provision, current_epoch);
        ...
        fn remove(ref self: Storage, amount: Wad) {
            ...
            if current_provider_shares.is_zero() {
                ...
                self
                    .provisions
                    .write(provider, Provision { epoch: current_epoch, shares: 0_u128.into() });
        ...
        fn update(
            ref self: Storage, mut assets: Span<ContractAddress>, mut asset_amts: Span<u128>
        ) {
            ...
            let rewards_count: u8 = self.rewards_count.read();
            self.bestow(current_epoch, rewards_count);
                    ...
                    Option::Some(asset) => {
                        ...
                        self
                            .update_absorbed_asset(
                                current_absorption_id, total_shares, *asset, asset_amt
                            );
                    },
                    Option::None(_) => {
                        break;
    ...
    impl StorageImpl of StorageTrait {
        ...
        #[inline(always)]
        fn yin_erc20(self: @Storage) -> IERC20Dispatcher {
        ...
        fn convert_to_shares(self: @Storage, yin_amt: Wad, round_up: bool) -> (Wad, Wad) {
            let total_shares: Wad = self.total_shares.read();
            ...
            let (computed_shares, r) = u128_safe_divmod(
                yin_amt.val * total_shares.val,
                yin_balance.val.try_into().expect('Division by zero')
            );
            if round_up & r != 0 {
        ...
        fn convert_to_yin(self: @Storage, shares_amt: Wad) -> Wad {
            let total_shares: Wad = self.total_shares.read();
        ...
        fn convert_epoch_shares(
            self: @Storage, start_epoch: u32, end_epoch: u32, start_shares: Wad
        ) -> Wad {
        ...
        fn update_absorbed_asset(
            ...
            self
                .asset_absorption
                .write((asset, absorption_id), DistributionInfo { asset_amt_per_share, error });
        }
        ...
        fn reap_internal(
            ref self: Storage, provider: ContractAddress, provision: Provision, current_epoch: u32
        ) {
            // Trigger issuance of rewards
            let rewards_count: u8 = self.rewards_count.read();
            self.bestow(current_epoch, rewards_count);
            ...
            let provider_last_absorption_id: u32 = self.provider_last_absorption.read(provider);
            let current_absorption_id: u32 = self.absorptions_count.read();
            self.provider_last_absorption.write(provider, current_absorption_id);
        ...
        fn get_shrine_ltv_to_threshold(self: @Storage) -> Ray {
        ...
        fn assert_can_remove(self: @Storage, request: Request) {
        ...
        fn bestow(ref self: Storage, epoch: u32, rewards_count: u8) {
                ...
                match assets.pop_front() {
                    Option::Some(asset) => {
                        ...
                        self
                            ...
                            .write((provider, *asset), epoch_reward_info.asset_amt_per_share)
                    },
                    Option::None(_) => {
                        break;
                    },
        ...
        fn propagate_reward_errors(ref self: Storage, rewards_count: u8, epoch: u32) {
            let mut current_rewards_id: u8 = 0;
