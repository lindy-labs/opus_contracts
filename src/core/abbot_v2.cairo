...
pub mod abbot_v2 {
    ...
    use opus::utils::erc721::erc721_component;
    use opus::utils::reentrancy_guard::reentrancy_guard_component;
    use opus::utils::src5::src5_component;
    ...
    #[abi(embed_v0)]
    impl ERC721MixinImpl = erc721_component::ERC721MixinImpl<ContractState>;
    ...
    fn constructor(
        ...
        symbol: ByteArray,
        base_uri: ByteArray,
        old_abbot: ContractAddress,
        shrine: ContractAddress,
        sentinel: ContractAddress
    ) {
        self.erc721.initializer(name, symbol, base_uri);

        let old_abbot = IAbbotDispatcher { contract_address: old_abbot };
        let total_troves = old_abbot.get_troves_count();

        let mut trove_id = 1;
        while trove_id <= total_troves {
            let owner = old_abbot.get_trove_owner(trove_id);
            self.erc721.mint(owner, trove_id.into());
    ...
    impl IAbbotImpl of IAbbot<ContractState> {
        ...
        fn get_trove_owner(self: @ContractState, trove_id: u64) -> Option<ContractAddress> {
            let owner = self.erc721.owner_of(trove_id.into());
            if owner.is_zero() {
                Option::None
            } else {
                Option::Some(owner)
        ...
        fn open_trove(
            ...
            self.erc721.mint(user, trove_id);
