...
mod Abbot {
    ...
    fn open_trove(
        ...
        // forge Yin
        shrine::read().forge(user, new_trove_id, forge_amount, max_forge_fee);
    ...
    #[external]
    fn forge(trove_id: u64, amount: Wad, max_forge_fee: Wad) {
        let user = get_caller_address();
        assert_trove_owner(user, trove_id);
        shrine::read().forge(user, trove_id, amount, max_forge_fee);
    }
