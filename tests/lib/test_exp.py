...
from tests.utils import assert_equalish_soft, compile_contract, from_wad, int_to_felt, to_wad
...
@pytest.mark.asyncio
async def test_exp_fail(deploy_test_contract):
    contract = deploy_test_contract
...
async def test_exponent_sum(deploy_test_contract):
    ...
    for case in test_input:
        ...
        result_sum = Decimal(
            from_wad((await contract.get_exp(int_to_felt(to_wad(case[0] + case[1]))).invoke()).result.res)
        )
