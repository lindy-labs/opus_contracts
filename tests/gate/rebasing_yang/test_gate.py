...
MOCK_SENTINEL = str_to_felt("abbot")
...
async def shrine_authed(starknet: Starknet, shrine, steth_token, wbtc_token) -> StarknetContract:
    ...
    await shrine.grant_role(role_value, MOCK_SENTINEL).execute(caller_address=SHRINE_OWNER)
...
async def trove_1_enter(shrine_authed, gate_info, funded_users) -> StarknetCallInfo:
    ...
    enter = await gate.enter(TROVE1_OWNER, TROVE_1, scaled_deposit_amt).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.deposit(token.contract_address, TROVE_1, yang_wad).execute(caller_address=MOCK_SENTINEL)
...
async def trove_2_enter_before_rebase(shrine_authed, gate_info, trove_1_enter) -> StarknetCallInfo:
    ...
    yang_wad = (await gate.preview_enter(scaled_deposit_amt).execute()).result.preview
    enter = await gate.enter(TROVE2_OWNER, TROVE_2, scaled_deposit_amt).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.deposit(token.contract_address, TROVE_2, yang_wad).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_subsequent_enter_with_rebase(shrine_authed, gate_info):
    ...
    enter = await gate.enter(TROVE1_OWNER, TROVE_1, deposit_amt).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.deposit(token.contract_address, TROVE_1, yang_wad).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_exit_before_rebase(shrine_authed, gate_info, collect_gas_cost):
    ...
    gate_exit = await gate.exit(TROVE1_OWNER, TROVE_1, FIRST_DEPOSIT_YANG).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_multi_user_exit_without_rebase(shrine_authed, gate_info):
    ...
    trove_2_gate_exit = await gate.exit(TROVE2_OWNER, TROVE_2, trove_2_yang).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.withdraw(token.contract_address, TROVE_2, trove_2_yang).execute(caller_address=MOCK_SENTINEL)
    ...
    # Withdraw from trove 1
    trove_1_gate_exit = await gate.exit(TROVE1_OWNER, TROVE_1, trove_1_yang).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.withdraw(token.contract_address, TROVE_1, trove_1_yang).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_multi_user_exit_with_rebase(shrine_authed, gate_info):
    ...
    # Withdraw from trove 2
    await gate.exit(TROVE2_OWNER, TROVE_2, trove_2_yang).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.withdraw(token.contract_address, TROVE_2, trove_2_yang).execute(caller_address=MOCK_SENTINEL)
    ...
    # Withdraw from trove 1
    await gate.exit(TROVE1_OWNER, TROVE_1, trove_1_yang).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.withdraw(token.contract_address, TROVE_1, trove_1_yang).execute(caller_address=MOCK_SENTINEL)
...
async def test_kill(shrine_authed, gate_info):
    ...
    with pytest.raises(StarkException, match="Gate: Gate is not live"):
        deposit_amt = to_fixed_point(SECOND_DEPOSIT_AMT, decimals)
        await gate.enter(TROVE1_OWNER, TROVE_1, deposit_amt).execute(caller_address=MOCK_SENTINEL)
    ...
    # Withdraw from trove 1
    await gate.exit(TROVE1_OWNER, TROVE_1, withdraw_yang_amt).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_enter_insufficient_fail(gate_info):
    ...
    with pytest.raises(StarkException, match="Gate: Transfer of asset failed"):
        ...
        await gate.enter(TROVE1_OWNER, TROVE_1, invalid_deposit_amt).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_exit_insufficient_fail(shrine_authed, gate_info):
    ...
    with pytest.raises(StarkException, match="Gate: Transfer of asset failed"):
        await gate.exit(TROVE1_OWNER, TROVE_1, FIRST_DEPOSIT_YANG + excess).execute(caller_address=MOCK_SENTINEL)
...
async def test_zero_enter_exit(shrine_authed, gate_info, fn):
    ...
    # Call test function
    await getattr(gate, fn)(TROVE1_OWNER, TROVE_1, 0).execute(caller_address=MOCK_SENTINEL)
...
async def test_gate_levy(shrine_authed, gate_info):
    ...
    levy = await gate.levy().execute(caller_address=MOCK_SENTINEL)
    ...
    # Exit
    await gate.exit(TROVE1_OWNER, TROVE_1, user_yang).execute(caller_address=MOCK_SENTINEL)
    await shrine_authed.withdraw(token.contract_address, TROVE_1, user_yang).execute(caller_address=MOCK_SENTINEL)
