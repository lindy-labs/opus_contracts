...
async def test_yin_transfer_fail(yin):
    ...
    with pytest.raises(StarkException, match="Shrine: Transfer amount exceeds yin balance"):
        await yin.transfer(USER_2, to_uint(FORGE_AMT_WAD + 1)).execute(caller_address=TROVE1_OWNER)

    # Attempting to transfer any amount of yin when USER_2 owns nothing
    with pytest.raises(StarkException, match="Shrine: Transfer amount exceeds yin balance"):
        await yin.transfer(TROVE1_OWNER, to_uint(1)).execute(caller_address=USER_2)


@pytest.mark.usefixtures("shrine_forge")
@pytest.mark.parametrize("shrine_both", ["shrine", "shrine_killed"], indirect=["shrine_both"])
@pytest.mark.asyncio
async def test_yin_transfer_from_pass(shrine_both, yin):

    # TROVE1_OWNER approves USER_2
    approve_tx = await yin.approve(USER_2, to_uint(FORGE_AMT_WAD)).execute(caller_address=TROVE1_OWNER)
    ...
    assert from_uint(allowance) == FORGE_AMT_WAD

    # USER_2 transfers all of TROVE1_OWNER's funds to USER_3
    await yin.transferFrom(TROVE1_OWNER, USER_3, to_uint(FORGE_AMT_WAD)).execute(caller_address=USER_2)

    # Checking balances
    u1_bal = (await yin.balanceOf(TROVE1_OWNER).execute()).result.balance
    assert from_uint(u1_bal) == 0

    u3_bal = (await yin.balanceOf(USER_3).execute()).result.balance
    assert from_uint(u3_bal) == FORGE_AMT_WAD

    # Checking USER_2's allowance
    u2_allowance = (await yin.allowance(TROVE1_OWNER, USER_2).execute()).result.allowance
    assert from_uint(u2_allowance) == 0


@pytest.mark.usefixtures("shrine_forge")
@pytest.mark.asyncio
async def test_yin_INFINITE_YIN_ALLOWANCE(yin):
    # infinite allowance test
    await yin.approve(USER_2, to_uint(INFINITE_YIN_ALLOWANCE)).execute(caller_address=TROVE1_OWNER)
    await yin.transferFrom(TROVE1_OWNER, USER_3, to_uint(FORGE_AMT_WAD)).execute(caller_address=USER_2)
    u2_allowance = (await yin.allowance(TROVE1_OWNER, USER_2).execute()).result.allowance
    assert from_uint(u2_allowance) == INFINITE_YIN_ALLOWANCE


@pytest.mark.usefixtures("shrine_forge")
@pytest.mark.asyncio
async def test_yin_transfer_from_fail(yin):
    ...
    await yin.approve(USER_2, to_uint(FORGE_AMT_WAD // 2)).execute(caller_address=TROVE1_OWNER)
    ...
    with pytest.raises(StarkException, match="Shrine: Transfer amount exceeds yin balance"):
        await yin.transferFrom(TROVE1_OWNER, USER_3, to_uint(FORGE_AMT_WAD + 1)).execute(caller_address=USER_2)
    ...
    with pytest.raises(StarkException, match="Yin: Cannot transfer to the zero address"):
        await yin.transferFrom(TROVE1_OWNER, 0, to_uint(FORGE_AMT_WAD)).execute(caller_address=USER_2)


@pytest.mark.parametrize("amount", [-1, 2**125 + 1, 2**128 + 1, 2**128 - 1])
@pytest.mark.asyncio
async def test_yin_transfer_invalid_inputs(yin, amount):
    with pytest.raises(StarkException):
        await yin.transfer(USER_2, to_uint(amount)).execute(caller_address=TROVE1_OWNER)


@pytest.mark.parametrize("amount", [-1, 2**256])
@pytest.mark.asyncio
async def test_yin_approve_invalid_inputs(yin, amount):
    with pytest.raises(StarkException, match="Yin: Amount not valid"):
        await yin.approve(USER_2, to_uint(amount)).execute(caller_address=TROVE1_OWNER)
...
@pytest.mark.parametrize("shrine_both", ["shrine", "shrine_killed"], indirect=["shrine_both"])
@pytest.mark.asyncio
async def test_yin_melt_after_transfer(shrine_both, yin):
    shrine = shrine_both

    # Transferring half of TROVE1_OWNER's balance to USER_2
    await yin.transfer(USER_2, to_uint(FORGE_AMT_WAD // 2)).execute(caller_address=TROVE1_OWNER)
...
async def test_flashFee(yin):
    assert (await yin.flashFee(yin.contract_address, to_uint(200)).execute()).result.fee == (0, 0)


@pytest.mark.asyncio
async def test_flashFee_unsupported_token(yin):
    with pytest.raises(StarkException, match="Yin: Unsupported token"):
        await yin.flashFee(0xDEADCA7, to_uint(3000)).execute()


@pytest.mark.usefixtures("shrine_forge")
@pytest.mark.asyncio
async def test_maxFlashLoan(yin):
    total_yin_uint = (await yin.totalSupply().execute()).result.total_supply
