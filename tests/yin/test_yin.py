...
async def shrine_killed(shrine, users) -> StarknetContract:
    shrine_owner = await users("shrine owner")
    await shrine_owner.send_tx(shrine, "kill", [])
    return shrine
...
async def test_yin_transfer_pass(shrine_forge, shrine_both, yin):
    ...
    u1_bal = (await yin.balanceOf(USER_1).invoke()).result.wad
    ...
    u1_new_bal = (await yin.balanceOf(USER_1).invoke()).result.wad
    assert u1_new_bal == 0
    ...
    # Attempting to transfer 0 yin when USER_1 owns nothing - should pass
    await yin.transfer(USER_2, 0).invoke(caller_address=USER_1)
...
async def test_yin_transfer_from_pass(shrine_forge, shrine_both, yin):
    ...
    allowance = (await yin.allowance(USER_1, USER_2).invoke()).result.wad
    ...
    u2_allowance = (await yin.allowance(USER_1, USER_2).invoke()).result.wad
...
async def test_yin_invalid_inputs(yin):
    ...
    with pytest.raises(StarkException, match=r"Yin: amount is not in the valid range \[0, 2\*\*125\]"):
        await yin.approve(USER_2, 2**128 + 1).invoke(caller_address=USER_1)

    with pytest.raises(StarkException, match=r"Yin: amount is not in the valid range \[0, 2\*\*125\]"):
        await yin.approve(USER_2, 2**128 - 1).invoke(caller_address=USER_1)

    with pytest.raises(StarkException, match=r"Yin: amount is not in the valid range \[0, 2\*\*125\]"):
        await yin.approve(USER_2, -2).invoke(caller_address=USER_1)

    with pytest.raises(StarkException, match=r"Yin: amount is not in the valid range \[0, 2\*\*125\]"):
        await yin.approve(USER_2, 2**125 + 1).invoke(caller_address=USER_1)
