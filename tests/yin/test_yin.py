...
async def test_yin_transfer_pass(shrine_forge, shrine_both, yin):
    ...
    transfer_tx = await yin.transfer(USER_2, FORGE_AMT).invoke(caller_address=TROVE1_OWNER)
    ...
    assert_event_emitted(transfer_tx, yin.contract_address, "Transfer", [TROVE1_OWNER, USER_2, FORGE_AMT])
...
async def test_yin_transfer_fail(shrine_forge, yin):
    ...
    with pytest.raises(StarkException, match="Shrine: transfer amount exceeds yin balance"):
        await yin.transfer(USER_2, FORGE_AMT + 1).invoke(caller_address=TROVE1_OWNER)
...
async def test_yin_transfer_from_pass(shrine_forge, shrine_both, yin):
    ...
    approve_tx = await yin.approve(USER_2, FORGE_AMT).invoke(caller_address=TROVE1_OWNER)
    ...
    await yin.transferFrom(TROVE1_OWNER, USER_3, FORGE_AMT).invoke(caller_address=USER_2)
...
async def test_yin_transfer_from_fail(shrine_forge, yin):
    ...
    # TROVE1_OWNER approves USER_2 but not enough to send FORGE_AMT
    await yin.approve(USER_2, FORGE_AMT // 2).invoke(caller_address=TROVE1_OWNER)

    # Should fail since USER_2's allowance for TROVE1_OWNER is less than FORGE_AMT
    with pytest.raises(StarkException, match="Yin: insufficient allowance"):
        await yin.transferFrom(TROVE1_OWNER, USER_3, FORGE_AMT).invoke(caller_address=USER_2)
    ...
    with pytest.raises(StarkException, match="Shrine: transfer amount exceeds yin balance"):
        await yin.transferFrom(TROVE1_OWNER, USER_3, FORGE_AMT + 1).invoke(caller_address=USER_2)
    ...
    with pytest.raises(StarkException, match="Yin: cannot transfer to the zero address"):
        await yin.transferFrom(TROVE1_OWNER, 0, FORGE_AMT).invoke(caller_address=USER_2)
...
async def test_yin_melt_after_transfer(shrine_forge, shrine_both, yin):
    ...
    await yin.transfer(USER_2, FORGE_AMT // 2).invoke(caller_address=TROVE1_OWNER)
