...
@pytest.fixture
async def shrine_deploy(starknet) -> StarknetContract:
    shrine_contract = compile_contract("contracts/shrine/shrine_iterative.cairo")
...
async def test_shrine_single_yang_one_redistribution(shrine):
    ...
    yang_pending_debt = (await shrine.get_yang_pending_debt_info(YANG1_ADDRESS).execute()).result.info
...
async def test_shrine_single_yang_two_redistribution(shrine):
    ...
    assert_equalish(from_wad(yang_pending_debt.total), from_wad(expected_redistributed_debt))
...
async def test_shrine_double_yang_one_redistribution(shrine):
    ...
    # - shrine_yang_pending_debt_info * 2
    ...
    yang1_pending_debt = (await shrine.get_yang_pending_debt_info(YANG1_ADDRESS).execute()).result.info
    ...
    yang2_pending_debt = (await shrine.get_yang_pending_debt_info(YANG2_ADDRESS).execute()).result.info
...
async def test_shrine_double_yang_two_redistributions(shrine):
    ...
    estimated_trove3_debt = from_wad((await shrine.estimate(TROVE_3).execute()).result.debt)

    # Simulate purge with 0 yin
    await shrine.melt(TROVE1_OWNER, TROVE_1, 0).execute(caller_address=SHRINE_OWNER)
    await shrine.redistribute(TROVE_1).execute(caller_address=MOCK_PURGER)

    updated_estimated_trove2_debt = from_wad((await shrine.estimate(TROVE_2).execute()).result.debt)
...
async def test_shrine_triple_yang_one_redistribution(shrine):
    ...
    yang3_pending_debt = (await shrine.get_yang_pending_debt_info(YANG3_ADDRESS).execute()).result.info
    ...
    # - shrine_total_debt (via `estimate`)
    # - shrine_troves (via `estimate`)
    # - shrine_yang_pending_debt_info * 3
    # - shrine_trove_redistribution
