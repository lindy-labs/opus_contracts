...
async def shrine_withdrawal(users, shrine_setup, shrine_deposit) -> StarknetTransactionExecutionInfo:
    ...
    withdrawal = await shrine_owner.send_tx(shrine.contract_address, "withdraw", [0, to_wad(10), shrine_user.address, 0])
    return withdrawal
...
async def test_shrine_setup(shrine_setup):
    ...
    live = (await shrine.get_is_live().invoke()).result.is_live
    assert live == 1
    ...
    # Check gages
    gage0 = (await shrine.get_gages(0).invoke()).result.gage
    assert gage0 == Gage(0, GAGES[0]["ceiling"])

    gage1 = (await shrine.get_gages(1).invoke()).result.gage
    assert gage1 == Gage(0, GAGES[1]["ceiling"])

    gage2 = (await shrine.get_gages(2).invoke()).result.gage
...
async def test_shrine_deposit(shrine_setup, users, shrine_deposit):
    ...
    gage = (await shrine.get_gages(0).invoke()).result.gage
